...
# - Password changes (4723): alert on password change events (no repeat for same RecordId)
# - Password resets (4724): alert on password reset events (per-account deduped; no repeat spam)
# - Process whitelist (4688): alert on non-whitelisted process creation (no repeat for same RecordId)
# - Privilege escalation (4672): alert on special privileges assigned (no repeat for same RecordId; ignores SYSTEM/SERVICE)
# - Mass file activity (4663): optional burst detection (requires Object Access auditing)
# - Security log cleared (1102): alert when Security log is cleared (critical)
# - Service installed (4697): alert on new service creation (persistence indicator)
#
...
WARMUP_LOOKBACK_MIN = 10

# Default process whitelist:
DEFAULT_WHITELIST = {
    # Core system / session processes
    "smss.exe", "csrss.exe", "wininit.exe", "services.exe", "lsass.exe",
    "lsm.exe", "winlogon.exe", "dwm.exe", "fontdrvhost.exe",
    "spoolsv.exe", "svchost.exe", "taskhostw.exe", "ctfmon.exe",
    "searchindexer.exe", "searchapp.exe", "shellexperiencehost.exe",
    "runtimebroker.exe", "systemsettings.exe",

    # Core UI / admin tools
    "explorer.exe", "notepad.exe", "wordpad.exe", "calc.exe",
    "mspaint.exe", "snippingtool.exe", "osk.exe", "control.exe",
    "mmc.exe", "taskmgr.exe", "regedit.exe", "eventvwr.exe",
    ...
    "onenote.exe", "acrobat.exe", "acrord32.exe",

    # Dev tools
    "code.exe", "python.exe", "py.exe",
    "pycharm64.exe", "idea64.exe",

    # Console host + this detector
    "conhost.exe", "eventlog_detector.py",
}
...
def ensure_admin():
    try:
        ...
        is_admin = False
    if not is_admin:
        print("[!] This tool must be run as Administrator. Re-open VS Code as Administrator.")
        sys.exit(1)
...
def load_or_create_key() -> bytes:
    if AUDIT_KEY_FILE.exists():
        ...
        if len(key) == 32:
            ...
            print("[!] Invalid existing key; regenerating.")
...
def fetch_security_events_by_id(event_id: int, minutes_back: int = 10) -> List[Dict[str, Any]]:
    ...
    script = (
        ...
        f"| Select-Object RecordId, Id, TimeCreated, ProviderName, Message "
...
RE_4672_SECID = re.compile(r"Security ID:\s*([^\r\n]+)", re.IGNORECASE)
...
RE_4697_SVCNAME = re.compile(r"Service Name:\s*([^\r\n]+)", re.IGNORECASE)
RE_4697_IMAGE = re.compile(r"Service File Name:\s*([^\r\n]+)", re.IGNORECASE)

RE_STATUS = re.compile(r"Status:\s*([0-9A-Fa-fx]+)", re.IGNORECASE)
RE_SUBSTATUS = re.compile(r"Sub Status:\s*([0-9A-Fa-fx]+)", re.IGNORECASE)

# Target account (for 4723/4724 dedup / context)
RE_472X_TARGET = re.compile(r"Target Account Name:\s*([^\r\n]+)", re.IGNORECASE)
...
class Detector:
    ...
    def load_whitelist(self):
        if WHITELIST_FILE.exists():
            try:
                ...
                print(f"[i] Loaded {len(extra)} extra whitelist entries")
    ...
    def _emit(self, event_id: int, category: str, description: str, when: dt.datetime, extras: Dict[str, Any] = None):
        ...
        print(f"[ALERT] EventID:{event_id} | Timestamp:{stamp} | Category:{category} | {description}")
        record = {
            "ts": stamp,
            "event_id": event_id,
            "category": category,
    ...
    def rule_failed_logins(self):
        ...
        if current_stage > self.failed_last_stage:
            self.failed_last_stage = current_stage
            desc = (
                f">5 failures in {FAILED_LOGIN_WINDOW_MIN} minutes (count={count})"
                if current_stage == 6 else
                f">={current_stage} failures in {FAILED_LOGIN_WINDOW_MIN} minutes (count={count})"
            )
            self._emit(
                4625,
                "Failed Logins",
    ...
    def rule_account_changes(self):
        for ev_id in (4720, 4726, 4728):
            ...
            for e in events:
                ...
                if rid in self.seen_record_ids:
                    continue
                t = parse_timestamp(e.get("TimeCreated", ""))
                msg = e.get("Message", "")
    ...
    def rule_password_changes(self):
        """
        4723: Password change event (user-side)
        4724: Password reset event (admin-side)

        We treat any 4723/4724 as "password-change-related activity worth alerting on".
        4724 is additionally deduplicated per target account using a cooldown
        to avoid double alerts from LSASS/SAM double-logging.
        """
        for ev_id in (4723, 4724):
            events = fetch_security_events_by_id(ev_id, minutes_back=10)
            for e in events:
                rid = str(e.get("RecordId", ""))
                if rid in self.seen_record_ids:
                ...
                status = extract_field(RE_STATUS, msg)
                sub_status = extract_field(RE_SUBSTATUS, msg)
                target_acct = extract_field(RE_472X_TARGET, msg)

                if ev_id == 4723:
                    category = "Password Change"
                    desc = "Account password change event."
                else:
                    category = "Password Reset"
                    desc = "Account password reset event."

                    # Deduplicate 4724 alerts per target account via cooldown
                    target_key = (target_acct or "UNKNOWN").upper()
                    cd_key = f"4724:{target_key}"
                    if self._should_cooldown(cd_key):
                        # Mark as seen so we don't retry next loop, but skip alert
                        self.seen_record_ids.add(rid)
                        continue
        ...
        for e in events:
            ...
            if newp not in self.whitelist:
                if not self._should_cooldown(f"4688:{newp}"):
                    self._emit(
                        4688,
                        "Process (Non-Whitelisted)",
                        f"New process: {newp} (parent: {parent or 'unknown'})",
                        t,
                        {"process": newp, "parent": parent},
                    )
            self.seen_record_ids.add(rid)
        ...
        for e in events:
            ...
            if not self._should_cooldown(f"4672:{subject_key}"):
                ...
                self._emit(
                    ...
                    t,
                    {"sid": sid, "account": acct, "domain": dom},
                )
        ...
        if len(self.mass_file_times) > MASS_FILE_THRESHOLD:
            if not self._should_cooldown("mass_file"):
                self._emit(
                    4663,
                    "Mass File Activity",
                    f">{MASS_FILE_THRESHOLD} file access events in {MASS_FILE_WINDOW_SECONDS}s "
                    f"(count={len(self.mass_file_times)})",
                    now,
                    {"count": len(self.mass_file_times)},
                )

    def rule_security_log_cleared(self):
        events = fetch_security_events_by_id(1102, minutes_back=60)
        for e in events:
            rid = str(e.get("RecordId", ""))
            if rid in self.seen_record_ids:
                continue
            ...
            if not self._should_cooldown("1102"):
                self._emit(
                    1102,
                    "Security Log",
                    "Security log was CLEARED — historical audit events were erased.",
                    t,
                    {"raw": msg[:400]},
                )
    ...
    def rule_service_installed(self):
        events = fetch_security_events_by_id(4697, minutes_back=60)
        for e in events:
            rid = str(e.get("RecordId", ""))
            if rid in self.seen_record_ids:
                continue
            ...
            svc_name = extract_field(RE_4697_SVCNAME, msg) or "UnknownService"
            image_path = extract_field(RE_4697_IMAGE, msg) or "UnknownImage"

            cd_key = f"4697:{svc_name}"
            if not self._should_cooldown(cd_key):
                desc = f"New service installed: {svc_name} -> {image_path}"
                self._emit(
                    4697,
                    "Service Installed",
                    desc,
                    t,
                    {"service_name": svc_name, "image_path": image_path, "raw": msg[:400]},
                )
    ...
    def run(self):
        ...
        print("    - Monitoring: 1102, 4625, 4720, 4723, 4724, 4726, 4728, 4672, 4688, 4663, 4697")
        ...
        while True:
            try:
                ...
                self.rule_password_changes()
                self.rule_privilege_escalation()
                self.rule_process_whitelist()
                self.rule_mass_file_activity()
                self.rule_security_log_cleared()
                self.rule_service_installed()
                time.sleep(POLL_INTERVAL_SECONDS)
            except KeyboardInterrupt:
                print("\n[i] Monitoring session ended.")
                break
            except Exception as e:
